...
public class CohereService {
  ...
  public botconversaChatTextRespose chat(botconversaChatRequestBody requestBody) throws Exception {
    ...
    return new botconversaChatTextRespose(responseBody.getText());
  }

  private static CohereChatBody createChatBody(botconversaChatRequestMessage[] messages) {
    ...
    for (int i = 0; i < messages.length - 1; i++) {
      botconversaChatRequestMessage message = messages[i];
  ...
  public botconversaChatTextRespose generateText(botconversaChatRequestBody requestBody) throws Exception {
    LOGGER.info("Received request body: {}", PrintObjectUtil.toJsonString(requestBody));
    ...
    // Sends response back to botconversa Chat using the Response format:
    // https://botconversachat.dev/docs/connect/#Response
    return new botconversaChatTextRespose(responseBody.getGenerations()[0].getText());
  }

  public botconversaChatTextRespose summarizeText(botconversaChatRequestBody requestBody) throws Exception {
    LOGGER.info("Received request body: {}", PrintObjectUtil.toJsonString(requestBody));
    ...
    // Sends response back to botconversa Chat using the Response format:
    // https://botconversachat.dev/docs/connect/#Response
    return new botconversaChatTextRespose(responseBody.getSummary());
  }
}
