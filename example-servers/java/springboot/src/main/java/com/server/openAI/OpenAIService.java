...
import com.server.utils.types.botconversaChatRequestMessage;
...
import com.server.utils.types.botconversaChatFile;
...
public class OpenAIService {
  ...
  public botconversaChatTextRespose chat(botconversaChatRequestBody requestBody) throws Exception {
    ...
    return new botconversaChatTextRespose(responseBody.getChoices()[0].getMessage().getContent());
  }

  @SuppressWarnings("unchecked")
  public Flux<botconversaChatTextRespose> chatStream(botconversaChatRequestBody requestBody) {
    LOGGER.info("Received request body: {}", PrintObjectUtil.toJsonString(requestBody));
      ...
      .concatMap(data -> {
        try {
          ...
          try {
            ...
            return Flux.just(new botconversaChatTextRespose(""));
          }
          ...
          // Sends response back to botconversa Chat using the Response format:
          // https://botconversachat.dev/docs/connect/#Response
          return Flux.just(new botconversaChatTextRespose(delta.toString()));
  ...
  private static OpenAIChatBody createOpenAIChatBody(botconversaChatRequestBody requestBody, Boolean stream) {
    ...
    botconversaChatRequestMessage[] requestMessages = requestBody.getMessages();
    OpenAIChatMessage[] opneAIChatMessages = new OpenAIChatMessage[requestMessages.length];
    for (int i = 0; i < requestMessages.length; i ++) {
      botconversaChatRequestMessage requestMessage = requestMessages[i];
  ...
  public botconversaChatFileResponse imageVariation(@RequestPart("files") List<MultipartFile> files) throws Exception {
    ...
    // Sends response back to botconversa Chat using the Response format:
    // https://botconversachat.dev/docs/connect/#Response
    return new botconversaChatFileResponse(Arrays.asList(new botconversaChatFile(responseData.getData()[0].getUrl(), "image")));
  }
}
