...
func createChatRequestBodyBytes(w http.ResponseWriter, r *http.Request, stream bool) ([]byte, error) {
	...
	chatBody := createChatBody(botconversaChatRequestBody, stream)
	return json.Marshal(chatBody)
}

func createChatBody(botconversaChatRequestBody botconversaChatRequestBody, stream bool) OpenAIChatBody {
	chatBody := OpenAIChatBody{
		Messages: make([]OpenAIChatMessage, len(botconversaChatRequestBody.Messages)),
		Model:    botconversaChatRequestBody.Model,
	}
	for i, message := range botconversaChatRequestBody.Messages {
		role := message.Role
...
// You can use an example image here: https://github.com/botconversa//blob/main/example-servers/ui/assets/example-image-for-openai.png
