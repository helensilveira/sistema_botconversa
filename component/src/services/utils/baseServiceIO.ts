...
export class BaseServiceIO implements ServiceIO {
  ...
  constructor(botconversaChat: botconversaChat, existingFileTypes?: ServiceFileTypes, demo?: DemoT) {
    this.botconversaChat = botconversaChat;
    this.demo = demo;
    Object.assign(this.rawBody, botconversaChat.connect?.additionalBodyProps);
    this.totalMessagesMaxCharLength = botconversaChat?.requestBodyLimits?.totalMessagesMaxCharLength;
    this.maxMessages = botconversaChat?.requestBodyLimits?.maxMessages;
    SetFileTypes.set(botconversaChat, this, existingFileTypes);
    if (botconversaChat.connect) this.connectSettings = botconversaChat.connect;
    if (this.demo) this.connectSettings.url ??= Demo.URL;
    if (this.connectSettings.websocket) Websocket.setup(this);
    this.stream = this.botconversaChat.connect?.stream || Legacy.checkForStream(this.botconversaChat);
  }
