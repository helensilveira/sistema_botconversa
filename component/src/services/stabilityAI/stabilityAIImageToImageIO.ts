...
export class StabilityAIImageToImageIO extends StabilityAIIO {
  ...
  constructor(botconversaChat: botconversaChat) {
    const directConnectionCopy = JSON.parse(JSON.stringify(botconversaChat.directConnection));
    const apiKey = directConnectionCopy.stabilityAI;
    const defaultFile = {images: {files: {acceptedFormats: '.png', maxNumberOfFiles: 1}}};
    super(botconversaChat, StabilityAIUtils.buildKeyVerificationDetails(), StabilityAIUtils.buildHeaders, apiKey, defaultFile);
