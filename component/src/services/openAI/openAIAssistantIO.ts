...
export class OpenAIAssistantIO extends DirectServiceIO {
  ...
  constructor(botconversaChat: botconversaChat) {
    const directConnectionCopy = JSON.parse(JSON.stringify(botconversaChat.directConnection)) as DirectConnection;
    const apiKey = directConnectionCopy.openAI;
    super(botconversaChat, OpenAIUtils.buildKeyVerificationDetails(), OpenAIUtils.buildHeaders, apiKey);
    ...
    if (typeof config === 'object') {
      ...
      const {function_handler} = botconversaChat.directConnection?.openAI?.assistant as OpenAIAssistant;
  ...
  private async fetchHistoryFunc() {
    setTimeout(() => this.botconversaChat.disableSubmitButton(), 2); // not initialised when fetchHistoryFunc called
    try {
      const threadMessages = await this.getThreadMessages(this.sessionId as string, true);
      this.botconversaChat.disableSubmitButton(false);
  ...
  private async getThreadMessages(thread_id: string, isHistory = false) {
    ...
    if (!isHistory && this.botconversaChat.responseInterceptor) {
      threadMessages = (await this.botconversaChat.responseInterceptor?.(threadMessages)) as OpenAIAssistantMessagesResult;
    }
  ...
  async extractPollResultData(result: OpenAIRunResult): PollResult {
    ...
    if (status === 'requires_action' && toolCalls) {
      ...
      return (await this.botconversaChat.responseInterceptor?.(toolsResult)) || toolsResult;
    }
  ...
  private async handleTools(toolCalls: ToolCalls): PollResult {
    if (!this._functionHandler) {
      throw Error(
        'Please define the `function_handler` property inside' +
          ' the [openAI](https://botconversachat.dev/docs/directConnection/openAI#Assistant) object.'
      );
    }
    ...
    if (!Array.isArray(handlerResponse) || handlerResponse.find((response) => typeof response !== 'string')
      ...
      throw Error('Response must contain an array of strings for each individual function/tool_call, '
        + 'see https://botconversachat.dev/docs/directConnection/OpenAI/#assistant-functions.');
    }
