...
export class OpenAIChatIO extends DirectServiceIO {
  ...
  constructor(botconversaChat: botconversaChat) {
    ...
    if (typeof config === 'object') {
      ...
      const {function_handler} = botconversaChat.directConnection?.openAI?.chat as OpenAIChat;
  ...
  private async handleTools(tools: ToolAPI, fetchFunc?: FetchFunc, prevBody?: OpenAIChat): Promise<ResponseT> {
    ...
    if (!tools.tool_calls || !fetchFunc || !prevBody || !this._functionHandler) {
      throw Error(
        'Please define the `function_handler` property inside' +
          ' the [openAI](https://botconversachat.dev/docs/directConnection/openAI#Chat) object.'
      );
    }
    ...
    if (handlerResponse.text) {
      ...
      return await this.botconversaChat.responseInterceptor?.(response) || response;
    }
    ...
    if ((Array.isArray(handlerResponse) && !handlerResponse.find((response) => typeof response !== 'string'))
      ...
      try {
        ...
        result = await this.botconversaChat.responseInterceptor?.(result) || result;
        if (result.error) throw result.error.message;
        return {text: result.choices[0].message.content || ''};
      } catch (e) {
    ...
    throw Error(
      'Response object must either be {response: string}[] for each individual function ' +
        'or {text: string} for a direct response, see https://botconversachat.dev/docs/directConnection/OpenAI#FunctionHandler.'
    );
  }
}
