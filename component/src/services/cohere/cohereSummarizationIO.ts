...
export class CohereSummarizationIO extends CohereIO {
  constructor(botconversaChat: botconversaChat) {
    const config = botconversaChat.directConnection?.cohere?.summarization as Cohere['summarization'];
    const apiKey = botconversaChat.directConnection?.cohere;
    super(botconversaChat, 'https://api.cohere.ai/v1/summarize', 'Insert text to summarize', config, apiKey);
  }
