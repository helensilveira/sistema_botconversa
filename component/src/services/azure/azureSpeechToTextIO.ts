...
export class AzureSpeechToTextIO extends AzureSpeechIO {
  ...
  constructor(botconversaChat: botconversaChat) {
    const config = botconversaChat.directConnection?.azure?.speechToText as NonNullable<Azure['speechToText']>;
    const apiKey = botconversaChat.directConnection?.azure;
    const defaultFile = {audio: {files: {acceptedFormats: '.wav,.ogg'}}};
    super(botconversaChat, AzureUtils.buildSpeechToTextHeaders, config.region, apiKey, defaultFile);
