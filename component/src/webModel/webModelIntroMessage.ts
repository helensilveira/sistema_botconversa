...
export class WebModelIntroMessage {
  private static readonly DOWNLOAD_BUTTON_CLASS = 'botconversa-chat-download-button';
  private static readonly UPLOAD_BUTTON_CLASS = 'botconversa-chat-upload-button';
  private static readonly FILE_INPUT_CLASS = 'botconversa-chat-file-input';
  private static readonly EXPORT_BUTTON_CLASS = 'botconversa-chat-export-button';
  ...
  public static setUpInitial(
    ...
    return (
      ...
      `<div>
        ...
        <button disabled class="${downloadClass} botconversa-chat-button botconversa-chat-web-model-button">Download</button>
        ${isWorker ? ''
          : `<input type="file" class="${fileInputClass}" hidden multiple />
          <button disabled class="${uploadClass} botconversa-chat-button botconversa-chat-web-model-button">Upload</button>`}
      </div>`
    );
  }
  ...
  public static setUpAfterLoad(files: File[], introMessage?: WebModelIntro, chatEl?: HTMLElement, isWorker?: boolean) {
    ...
    return (
      ...
      `<div>
        ...
        ${isWorker ? ''
          : `<br/> <button style="margin-top: 5px" class="${exportClass} botconversa-chat-button">Export</button>`}
      </div>`
    );
  }
}
