...
// import * as WebLLM2 from 'botconversa-chat-web-llm';
...
export class WebModel extends BaseServiceIO {
  ...
  private static readonly GENERIC_ERROR =
    ...
    '[troubleshooting](https://botconversachat.dev/docs/webModel#troubleshooting) section of documentation for help.';
  ...
  constructor(botconversaChat: botconversaChat) {
    ...
    if (typeof botconversaChat.webModel === 'object') this._webModel = botconversaChat.webModel;
    if (this._webModel.load?.clearCache) WebModel.clearAllCache();
    this.findModelInWindow(botconversaChat);
    this.canSendMessage = this.canSubmit.bind(this);
    this._chatEl = botconversaChat.shadowRoot?.children[0] as HTMLElement;
    if (botconversaChat.history) WebModel.setUpHistory(this._conversationHistory, botconversaChat.history);
  }
  ...
  private findModelInWindow(botconversaChat: botconversaChat, seconds = 0) {
    if (window.webLLM) {
      this.configureInit(this.shouldAddIntroMessage(botconversaChat.introMessage));
    } else if (seconds > WebModel.MODULE_SEARCH_LIMIT_S) {
      this._messages?.addNewErrorMessage('service', WebModel.WEB_LLM_NOT_FOUND_ERROR);
      console.error(
        'The botconversa-chat-web-llm module has not been attached to the window object. ' + 'Please see the following guide:'
      );
      console.error('https://botconversachat.dev/examples/externalModules');
    } else {
      setTimeout(() => this.findModelInWindow(botconversaChat, seconds + 1), 1000);
    }
  }
  ...
  private async loadModel(chat: WebLLM.ChatInterface, files?: FileList) {
    ...
    this.botconversaChat._validationHandler?.();
  ...
  private async immediateResp(messages: Messages, text: string, chat: WebLLM.ChatInterface) {
    ...
    const response = await WebModel.processResponse(this.botconversaChat, messages, output);
  ...
  private async streamResp(messages: Messages, text: string, chat: WebLLM.ChatInterface) {
    ...
    await chat.generate(text, async (_: number, message: string) => {
      const response = await WebModel.processResponse(this.botconversaChat, messages, {text: message});
  ...
  private async generateResp(messages: Messages, pMessages: MessageContentI[], chat: WebLLM.ChatInterface) {
    ...
    const {body, error} = await RequestUtils.processRequestInterceptor(this.botconversaChat, {body: {text: lastText}});
  ...
  private static async processResponse(botconversaChat: botconversaChat, messages: Messages, output: ResponseI) {
    const result: ResponseI = (await botconversaChat.responseInterceptor?.(output)) || output;
    if (result.error) {
      RequestUtils.displayError(messages, new Error(result.error));
      return;
    } else if (!result || !result.text) {
      const error = ErrorMessages.INVALID_MODEL_RESPONSE(output, !!botconversaChat.responseInterceptor, result);
      RequestUtils.displayError(messages, new Error(error));
      return;
    }
    return result;
  ...
  private static clearAllCache() {
    // IMPORTANT - 'webllm/model' and 'webllm/wasm' need to match the scope in 'botconversa-chat-web-llm':
