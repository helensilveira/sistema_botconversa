...
export class SpeechToText extends MicrophoneButton {
  ...
  constructor(botconversaChat: botconversaChat, textInput: TextInputEl, addErrorMessage: AddErrorMessage) {
    super(typeof botconversaChat.speechToText === 'object' ? botconversaChat.speechToText?.button : {});
    const {serviceName, processedConfig} = this.processConfiguration(textInput, botconversaChat.speechToText);
    ...
    if (serviceName === 'webspeech' && !SpeechToElement.isWebSpeechSupported()) {
      ...
      const isInputEnabled = !botconversaChat.textInput || !botconversaChat.textInput.disabled;
