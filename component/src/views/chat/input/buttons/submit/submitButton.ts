...
export class SubmitButton extends InputButton<Styles> {
  ...
  // prettier-ignore
  constructor(botconversaChat: botconversaChat, inputElementRef: HTMLElement, messages: Messages, serviceIO: ServiceIO,
      fileAttachments: FileAttachments, buttons: Buttons) {
    const submitButtonStyles = SubmitButtonStateStyle.process(botconversaChat.submitButtonStyles);
    ...
    botconversaChat.disableSubmitButton = this.disableSubmitButton.bind(this, serviceIO);
    this.attemptOverwriteLoadingStyle(botconversaChat);
    if (buttons.microphone) this.setUpSpeechToText(buttons.microphone.button, botconversaChat.speechToText);
    setTimeout(() => { // in a timeout as botconversaChat._validationHandler initialised later
      this._validationHandler = botconversaChat._validationHandler;
  ...
  // prettier-ignore
  private attemptOverwriteLoadingStyle(botconversaChat: botconversaChat) {
    if (this._customStyles?.submit?.svg
        || this._customStyles?.loading?.svg?.content || this._customStyles?.loading?.text?.content) return;
    if (botconversaChat.displayLoadingBubble === undefined || botconversaChat.displayLoadingBubble === true) {
      ...
      botconversaChat.shadowRoot?.appendChild(styleElement);
  ...
  private setUpSpeechToText(microphoneButton: InputButton, speechToText: botconversaChat['speechToText']) {
