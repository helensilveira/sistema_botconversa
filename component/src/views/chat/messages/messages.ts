...
import {HTMLbotconversaChatElements} from './html/htmlbotconversaChatElements';
...
export class Messages extends MessagesBase {
  ...
  constructor(botconversaChat: botconversaChat, serviceIO: ServiceIO, panel?: HTMLElement) {
    super(botconversaChat);
    const {permittedErrorPrefixes, introPanelMarkUp, demo} = serviceIO;
    this._errorMessageOverrides = botconversaChat.errorMessages?.overrides;
    this._onClearMessages = FireEvents.onClearMessages.bind(this, botconversaChat);
    this._onError = FireEvents.onError.bind(this, botconversaChat);
    this._displayLoadingMessage = Messages.getDisplayLoadingMessage(botconversaChat, serviceIO);
    this._permittedErrorPrefixes = permittedErrorPrefixes;
    if (!this.addSetupMessageIfNeeded(botconversaChat, serviceIO)) {
      this.populateIntroPanel(panel, introPanelMarkUp, botconversaChat.introPanelStyle);
    }
    this.addIntroductoryMessage(botconversaChat, serviceIO);
    this.populateHistory(botconversaChat);
    this._displayServiceErrorMessages = botconversaChat.errorMessages?.displayServiceErrorMessages;
    botconversaChat.getMessages = () => JSON.parse(JSON.stringify(this.messages));
    botconversaChat.clearMessages = this.clearMessages.bind(this, serviceIO);
    botconversaChat.refreshMessages = this.refreshTextMessages.bind(this);
    botconversaChat.scrollToBottom = ElementUtils.scrollToBottom.bind(this, this.elementRef);
    botconversaChat.addMessage = (message: ResponseI, isUpdate?: boolean) => {
    ...
    if (botconversaChat.textToSpeech) {
      TextToSpeech.processConfig(botconversaChat.textToSpeech, (processedConfig) => {
  ...
  private static getDisplayLoadingMessage(botconversaChat: botconversaChat, serviceIO: ServiceIO) {
    if (serviceIO.websocket) return false;
    return botconversaChat.displayLoadingBubble ?? true;
  }
  ...
  private addSetupMessageIfNeeded(botconversaChat: botconversaChat, serviceIO: ServiceIO) {
    const text = SetupMessages.getText(botconversaChat, serviceIO);
    if (text) {
  ...
  // WORK - const file for botconversa chat classes
  private addIntroductoryMessage(botconversaChat?: botconversaChat, serviceIO?: ServiceIO) {
    if (botconversaChat?.shadowRoot) this._introMessage = botconversaChat.introMessage;
    ...
    if (introMessage) {
      ...
      if (elements) {
        ...
        elements.outerContainer.classList.add('botconversa-chat-intro');
      }
    }
  }
  ...
  public removeIntroductoryMessage() {
    ...
    if (introMessage.outerContainer.classList.contains('botconversa-chat-intro')) {
      introMessage.outerContainer.remove();
      this.messageElementRefs.shift();
    }
  }

  private populateHistory(botconversaChat: botconversaChat) {
    const history = botconversaChat.history || Legacy.processHistory(botconversaChat);
    if (!history) return;
    ...
    // https://github.com/botconversa//issues/84
  ...
  public addNewMessage(data: ResponseI, isHistory = false) {
    ...
    if (message.html !== undefined && message.html !== null) {
      ...
      if (HTMLbotconversaChatElements.isElementTemporary(elements)) delete message.html;
    }
  ...
  // WORK - update all message classes to use botconversa-chat prefix
