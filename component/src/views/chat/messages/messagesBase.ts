...
export class MessagesBase {
  ...
  constructor(botconversaChat: botconversaChat) {
    ...
    this.messageStyles = botconversaChat.messageStyles;
    this._remarkable = RemarkableConfig.createNew();
    this._avatars = botconversaChat.avatars;
    this._names = botconversaChat.names;
    this._onMessage = FireEvents.onMessage.bind(this, botconversaChat);
    if (botconversaChat.htmlClassUtilities) this.htmlClassUtilities = botconversaChat.htmlClassUtilities;
    setTimeout(() => {
      this.submitUserMessage = botconversaChat.submitUserMessage; // wait for it to be available
    });
  }
  ...
  protected static isTemporaryElement(elements: MessageElements) {
    return (
      ...
      HTMLbotconversaChatElements.isElementTemporary(elements)
    );
  }
