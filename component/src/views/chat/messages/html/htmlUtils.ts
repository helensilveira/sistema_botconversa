...
import {HTMLbotconversaChatElements} from './htmlbotconversaChatElements';
...
export class HTMLUtils {
  ...
  private static applyClassUtilitiesToElement(element: HTMLElement, classUtility: HTMLClassUtility) {
    ...
    // if botconversa chat class then style was already applied
    if (styles && !HTMLbotconversaChatElements.doesElementContainbotconversaChatClass(element)) {
  ...
  public static apply(messages: MessagesBase, outmostElement: HTMLElement) {
    HTMLbotconversaChatElements.applybotconversaChatUtilities(messages, messages.htmlClassUtilities, outmostElement);
