...
export class Websocket {
  ...
  public static createConnection(io: ServiceIO, messages: Messages) {
    if (!document.body.contains(io.botconversaChat)) return; // check if element is still present
    ...
    try {
      ...
      io.websocket.onopen = () => {
        ...
        io.botconversaChat._validationHandler?.();
      };
  ...
  private static assignListeners(io: ServiceIO, ws: WebSocket, messages: Messages) {
    ...
    ws.onmessage = async (message) => {
      ...
      try {
        ...
        const finalResult = (await io.botconversaChat.responseInterceptor?.(result)) || result;
        const resultData = await io.extractResultData(finalResult);
        if (!resultData || typeof resultData !== 'object')
          throw Error(ErrorMessages.INVALID_RESPONSE(result, 'server', !!io.botconversaChat.responseInterceptor, finalResult));
  ...
  public static async sendWebsocket(io: ServiceIO, body: object, messages: Messages, stringifyBody = true) {
    ...
    const {body: interceptedBody, error} = await RequestUtils.processRequestInterceptor(io.botconversaChat, requestDetails);
