...
export class CustomHandler {
  public static async request(io: ServiceIO, body: RequestDetails['body'], messages: Messages) {
    ...
    const onResponse = async (response: Response) => {
      ...
      const result = (await io.botconversaChat.responseInterceptor?.(response)) || response;
      if (!RequestUtils.validateResponseFormat(result)) {
        console.error(ErrorMessages.INVALID_RESPONSE(response, 'server', !!io.botconversaChat.responseInterceptor, result));
