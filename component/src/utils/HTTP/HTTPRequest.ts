...
export class HTTPRequest {
  ...
  public static async request(io: ServiceIO, body: object, messages: Messages, stringifyBody = true) {
    ...
    const {body: interceptedBody, headers, error} =
      (await RequestUtils.processRequestInterceptor(io.botconversaChat, requestDetails));
    ...
    fetchFunc(interceptedBody).then((response) => {
      ...
      .then(async (result: Response) => {
        ...
        if (!resultData || typeof resultData !== 'object')
          throw Error(ErrorMessages.INVALID_RESPONSE(result, 'response', !!io.botconversaChat.responseInterceptor, finalResult));
  ...
  public static executePollRequest(io: ServiceIO, url: string, requestInit: RequestInit, messages: Messages) {
    ...
    fetch(url, requestInit)
      ...
      .then(async (result: object) => {
        ...
        const resultData = await io.extractPollResultData((await io.botconversaChat.responseInterceptor?.(result)) || result);
