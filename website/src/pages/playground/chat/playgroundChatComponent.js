import botconversaChatBrowser from '../../../components/table/botconversaChatBrowser';
import {useColorMode} from '@botconversa/theme-common';
...
export default function ChatComponent({config}) {
  ...
  function newMessage({isInitial}) {
    if (!isInitial) {
      const botconversaChatComponent = componentRef.current.children[0];
      if (config.connect?.openAI?.assistant) assignOpenAIAssistantId(botconversaChatComponent, config);
      const {messages} = config;
      messages.splice(0, messages.length);
      messages.push(...botconversaChatComponent.getMessages());
    }
  }
...
function assignOpenAIAssistantId(botconversaChatComponent, config) {
  if (botconversaChatComponent._activeService.rawBody.assistant_id) {
    if (typeof config.connect.openAI.assistant === 'boolean') {
      config.connect.openAI.assistant = {assistant_id: botconversaChatComponent._activeService.rawBody.assistant_id};
