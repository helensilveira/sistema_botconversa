    ...
    setTimeout(() => {
      if (containerRef.current) {
        ...
        if (containerRef.current && eventTextRef.current) {
          const botconversaChatReference = extractChildChatElement(containerRef.current.children[0]);
          botconversaChatReference[propertyName] = eventTextRef.current?.updateText;
        } else {
          const botconversaChatReference = extractChildChatElement(syncReference.children[0]);
          botconversaChatReference[propertyName] = () => {};
        }
      }
