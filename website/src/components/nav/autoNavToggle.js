...
// probably not the best way of toggling the scrollbar, but did not want to spend more time to override botconversa API
...
export function fadeIn() {
  setTimeout(() => {
    const element = document.querySelectorAll('.homepage > body > #__botconversa > nav')?.[0];
    try {
...
//   const targetElement = document.querySelector('.homepage > body > #__botconversa > nav');
...
//           if (node instanceof HTMLElement && node.matches('.homepage > body > #__botconversa > nav')) {
