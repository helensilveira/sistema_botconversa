...
<ComponentContainerEvents propertyName={'onNewMessage'}>
  <botconversaChatBrowser style={{borderRadius: '8px'}} demo={true}></botconversaChatBrowser>
...
- Function: (`chatElementRef: botconversaChat`) => `void`
- Event: `render`
...
:::note
Setting a property's value on the botconversaChat element when this event is triggered can cause infinite recursion
as the component will re-render every time a property value is set. Use a flag variable to prevent this - [example](https://github.com/botconversa//issues/99#issuecomment-1899202984).
:::
...
<ComponentContainerEvents propertyName={'onError'}>
  <botconversaChatBrowser style={{borderRadius: '8px'}} demo={{response: {error: 'custom error'}}}></botconversaChatBrowser>
</ComponentContainerEvents>
