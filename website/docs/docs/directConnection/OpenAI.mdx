...
When set to `true` or the `assistant_id` is not defined, botconversa Chat will automatically create a new assistant when the user sends the first message. <br />
...
When `assistant_id` is not used, this object is used to define the details of the new assistant that will be created by botconversa Chat when
...
Try it out live in the botconversa Chat [Playground](/playground).
