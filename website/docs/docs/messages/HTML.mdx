...
<BrowserOnly>
  {() => {
    require('active-table-react').botconversaChat;
  }}
</BrowserOnly>
<BrowserOnly>
  {() => {
...
```html
<botconversa-chat initialMessages='[{"html": "<button>Button</button>", "role": "user"}]'></botconversa-chat>
```
...
Because botconversa Chat is a [shadow element](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM) and your html is rendered inside it - the resulting elements will not be able to access the CSS and JavaScript
...
### botconversa Chat Classes {#botconversaChatClasses}

botconversa Chat comes with a pre-defined set of classes that can be used to add styling/functionality to your elements. <br />
`botconversa-chat-button` - applies default button styling. <br />
`botconversa-chat-suggestion-button` - when clicked sends the text that is inside the button as a new user message. <br />
`botconversa-chat-temporary-message` - removes the message when a new one is added to the chat. This class needs to be applied
...
<ComponentContainer>
  <botconversaChatBrowser
    initialMessages={[
      {html: '<button class="botconversa-chat-button botconversa-chat-suggestion-button">Hello</button>', role: 'ai'},
      {
        html: '<button class="botconversa-chat-suggestion-button botconversa-chat-temporary-message">Goodbye</button>',
        role: 'user',
      },
    ]}
    style={{height: '370px', borderRadius: '8px'}}
...
chatElementRef.initialMessages = [
  {html: '<button class="botconversa-chat-button botconversa-chat-suggestion-button">Hello</button>', role: 'ai'},
  {html: '<button class="botconversa-chat-suggestion-button botconversa-chat-temporary-message">Goodbye</button>', role: 'user'},
];
```
...
:::info
If you have suggestions for new classes that you think would be useful, please raise an [issue](https://github.com/botconversa//issues) on GitHub.
:::
...
```html
<botconversa-chat messageStyles='{"html": {"shared": {"bubble": {"backgroundColor": "unset"}}}}'></botconversa-chat>
```
...
- Web components can ultimately be created in any way you desire using any framework outside of your project and imported the same ways as you do
  `botconversa-chat`. To note, this approach may require you to first import or use them in your app's code in order for your bundler to register them.
...
// HTML
<botconversa-chat initialMessages='[{"html": "<custom-element/>", "role": "user"}]'></botconversa-chat>
```
...
:::info
If you are experiencing problems with embedding your custom elements inside the chat, you can always raise an [issue](https://github.com/botconversa//issues) on GitHub.
...
Basic suggestion buttons using [botconversa Chat Classes](#botconversaChatClasses).
...
<ComponentContainer>
  <botconversaChatBrowser
    ...
    initialMessages={[
      ...
      {
        html: `
          <div class="botconversa-chat-temporary-message">
            <button class="botconversa-chat-button botconversa-chat-suggestion-button" style="margin-top: 5px">What do shrimps eat?</button>
            <button class="botconversa-chat-button botconversa-chat-suggestion-button" style="margin-top: 6px">Can a shrimp fry rice?</button>
            <button class="botconversa-chat-button botconversa-chat-suggestion-button" style="margin-top: 6px">What is a pistol shrimp?</button>
          </div>`,
        role: 'ai',
      },
    ]}
  ...
  <botconversaChatBrowser
    ...
    initialMessages={[
      ...
      {text: 'Have you drank water?', role: 'ai'},
      {
        html: `
          <div class="botconversa-chat-temporary-message">
            <button class="botconversa-chat-button botconversa-chat-suggestion-button" style="border: 1px solid green">Yes</button>
            <button class="botconversa-chat-button botconversa-chat-suggestion-button" style="border: 1px solid #d80000">No</button>
          </div>`,
        role: 'user',
      },
    ]}
...
Add a chart component (e.g. using [Google Chart](https://developers.google.com/chart)). Live example for [React](https://codesandbox.io/s/botconversa-chat-react-chart-ztsgrd?file=/src/App.tsx).
