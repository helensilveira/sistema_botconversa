...
The outgoing botconversa Chat request body is encapsulated in one of the following formats:
...
the server. See [`example`](https://botconversachat.dev/examples/servers) server code. <br />
...
```html
<botconversa-chat stream="true"></botconversa-chat>
```
...
```html
<botconversa-chat stream='{"simulation": 6}'></botconversa-chat>
```
...
It is important to note that exchanged messages must be Stringified JSONs where botconversa Chat will send its messages using the [Request message](connect) format
and the server must send its messages using the [`Response`](#Response) format. Example messages: <br />
botconversa Chat message: <br />
`'{"messages":[{"role":"user","text":"Message from botconversa Chat"}]}'` <br />
Server message: <br />
...
```html
<botconversa-chat request='{"url": "ws://customapi.com", "websocket": true}'></botconversa-chat>
```
...
:::tip
Check the [websocket server template](https://github.com/botconversa//tree/main/example-servers/node/websockets) to help you get started.
:::
...
`signals` is a map of functions which are used to notify botconversa Chat on the status of the request and its result. The available
...
chatElementRef.request = {
  ...
  handler: (_, signals) => {
    try {
      ...
      websocket.onerror = () => {
        // 'Connection error' is a special string that will also display in botconversa Chat
...
Object containing functions that are used to notify the botconversa Chat component about the status of the current request.
The `stopClicked` and `newUserMessage` functions are triggered by botconversa Chat itself and contain `listener` properties
